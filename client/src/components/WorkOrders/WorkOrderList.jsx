...
import { getUserProfiles, getUserProfilesWithRoles, promoteUser } from "../../managers/userProfileManager";
...
export default function WorkOrderList({ loggedInUser }) {
  ...
  const completeWorkOrder = (workOrderId) => {
    completeWorkOrderRequest(workOrderId)
        .then (() => {
            getIncompleteWorkOrders().then(setWorkOrders);
        });
  };
  ...
  const deleteWorkOrder = (workOrderId) => {
    deleteWorkOrderRequest(workOrderId)
        .then(() => {
            getIncompleteWorkOrders().then(setWorkOrders);
        });
  };
